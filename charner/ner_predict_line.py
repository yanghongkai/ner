...
def test_evaluate(sess,logits_op,length_op,transMatrix_op,x,tX,xlength,vob,model_out):
    logits_,_,transMatrix_=sess.run([logits_op,length_op,transMatrix_op],feed_dict={x:tX })
    # logits_ shape [batch_size,80,16] [3,80,16]
    #print(logits_.shape)
    #print(transMatrix_.shape)
    # transMatrix shape [16,16]
    for unary_scores_,length_ ,input_ in zip(logits_,xlength,tX):
        if length_==0:
            continue
